...
def extraer_texto_ocr(archivo, resolucion):
    ...
    try:
        ...
        logging.error(f"Error al convertir el PDF en imágenes: {e}")
        return ""
    logging.info("PDF convertido a imagenes")

    # extract text
    text_data = ''
    for i, page in enumerate(pages):
        try:
            logging.info(f'Procesando página {i+1}...')
            text = pytesseract.image_to_string(page)
            logging.info(f'Texto extraído de la página {i+1}:\n{text}')
            text_data += text + '\n'
        except Exception as e:
            logging.error(f"Error al procesar la página {i+1} con OCR: {e}")
    return text_data
    ...
    with pdfplumber.open(archivo) as pdf_file:
        ...
        for num_pagina in range(len(pdf_file.pages)):
            ...
            if texto:
                ...
                print(f"No se pudo extraer texto de la página {num_pagina + 1}")
