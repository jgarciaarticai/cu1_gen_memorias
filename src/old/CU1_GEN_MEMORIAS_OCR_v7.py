...
try:
    ...
    # LECTURA OCR Y SPLIT DEL DOCUMENTO
    logger.info(f"El archivo que se va a leer es: {archivo_entrada}")
    data = OCR.ocr_fichero(archivo_entrada, 600)
    ...
    vectorstore = FAISS.from_texts(texts=all_splits, embedding=local_embeddings)
    logger.info(f"Base de datos vectorial inicializada correctamente.")
    
    retriever = vectorstore.as_retriever(search_type="similarity_score_threshold", search_kwargs={"k": int(config["k"]), "score_threshold": float(config["score_threshold"])})
    ...
    logger.info(f"Chain de RAG creada correctamente.")
    ...
    for _, row in queries_df.iterrows():
        ...
        try:
            ...
            documentos_comprimidos = compression_retriever.invoke(question)
            logger.info(f"RETRIEVED DOCS: {documentos_comprimidos}")

            respuesta = qa_chain.invoke({"context": documentos_comprimidos, "question": question})
