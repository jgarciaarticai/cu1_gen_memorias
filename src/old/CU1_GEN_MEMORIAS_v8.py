...
try:
    ...
    for filename in os.listdir(carpeta_entrada):
        ...
        if filename.endswith(".pdf"):
            ...
            #data = loader.load()
            data = OCR.procesar_pdf(filepath, 600) # OCR
            ...
            all_splits = text_splitter.split_text(data) # OCR
            logger.info(f"Documento {filename} dividido en chunks correctamente.")

            # Añadir los chunks al índice FAISS
            #vectorstore.add_documents(documents=all_splits, embedding=local_embeddings)
            vectorstore.add_texts(texts=all_splits, embedding=local_embeddings) # OCR
